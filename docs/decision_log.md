# 🏗️ 건설 도메인 지식 & 상식 Q&A 챗봇 서비스 - 아키텍처 결정 기록 및 학습 가이드 (ADR)
...
- **프로젝트명**: 건설 도메인 지식 & 상식 Q&A 챗봇 서비스 (`chat-be` & `chat-fe`)
- **타겟 도메인**: 건설 실무 및 일반 상식 7대 카테고리 (시공/공정, 인허가, 계약/비용, 참여주체, 자재/구조, 개념비교, 플랜트)
- **핵심 기술 스택**:
